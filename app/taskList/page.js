  ...
  useEffect(() => {
    const fetchAllTasks = async () => {
      try {
        const token = localStorage.getItem("authToken");
        const response = await axios.get(
          "http://localhost:5000/api/task/list",
          {
            headers: {
              Authorization: token,
            },
              ...
              response.data.tasks.map(async (task) => {
                try {
                  const employeeResponse = await axios.get(
                    `http://localhost:5000/api/subemployee/${task.assignTo}`,
                    {
                      headers: {
                        Authorization: token,
                      },
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:5000/api/task/delete/${deleteTaskId}`,
        {
          headers: {
            Authorization: token,
          },
  ...
  const handleViewClick = async (taskId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const employeeResponse = await axios.get(
          `http://localhost:5000/api/subemployee/${taskData.assignTo}`,
          {
            headers: {
              Authorization: token,
            },
  ...
  return (
    <>
      ...
      <div className="m-5 pl-0 md:pl-64 mt-20">
        <h1 className="text-xl md:text-2xl font-bold mb-4 text-orange-500 text-center md:text-left">
          All Tasks
        </h1>
                  ...
                  filteredTasks.map((task) => (
                    <tr key={task._id} className="hover:bg-gray-100">
                      ...
                      <td className="text-center border px-2 py-1">
                        <span
                          className={`rounded-full font-semibold px-5 py-1 ${
          ...
          >
            ...
            >
              <div className="p-2 text-center text-sm md:text-base">
                <h3 className="mb-4 text-xl font-semibold text-gray-800 dark:text-gray-400 text-center">
                  Task Details
                </h3>
                {viewTask && (
                  <div>
            ...
            >
              ...
              <div className="p-2 text-center">
                ...
                <FontAwesomeIcon
                  ...
                  className="text-3xl md:text-5xl text-orange-600 mt-2"
                />
